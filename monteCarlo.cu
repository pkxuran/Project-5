	      ...
	      float tmin = t1 < t2 ? t1 : t2; // only care about the first intersection
	        ...
	        ny /= nxy; // unit vector
	        	...
	        	float tt = (0. - ycir) / outy;
	       	 	...
	       	 	if (tt >= 0) {
	          	// this beam hit the infinite plate. (Case D) Increment the number
            ...
            for (int offset = 1; offset < wgDimension; offset *= 2)
            {
                int mask = 2 * offset - 1;
                __syncthreads();
                if ((threadNum & mask) == 0)
                {
                    numhits[threadNum] += numhits[threadNum + offset];
                }
            }

            __syncthreads();
            if (threadNum == 0)
            Hits[wgNumber] = numhits[0];
}
